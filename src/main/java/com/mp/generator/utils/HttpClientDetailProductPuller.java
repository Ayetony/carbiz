...
public class HttpClientDetailProductPuller {
    ...
    public static void main(String[] args) {
        //产品详情解析
        new HttpClientDetailProductPuller().productInfoFromJson("629417127052");//533816674053 614252193570
...
//            return null;
//        }
        Map<String, String> propListMap = null ;
        if(elementFlag != null && elementFlag.isJsonObject()) {
            propListMap = mapJson(element, "props_list");
        }
        ...
        if(propListMap != null) {
            ...
            if (img == null || !img.isJsonObject()) {
                ...
                Map<String, String> finalPropListMap = propListMap;
                propImgMap.forEach((key, value) -> {
                    if (finalPropListMap.get(key) != null) imgPropMap.put(finalPropListMap.get(key), value);
                });
                element.getAsJsonObject().get("skus").getAsJsonObject().get("sku").
                        getAsJsonArray().forEach(e -> {
                    String name = e.getAsJsonObject().get("properties_name").getAsString();
